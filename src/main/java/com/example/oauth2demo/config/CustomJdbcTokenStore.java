...
import com.example.oauth2demo.config.CustomAuthenticationKeyGenerator;
...
public class CustomJdbcTokenStore implements TokenStore {
    ...
    private AuthenticationKeyGenerator customAuthenticationKeyGenerator = new CustomAuthenticationKeyGenerator();
    ...
    public void setAuthenticationKeyGenerator(CustomAuthenticationKeyGenerator authenticationKeyGenerator) {
    ...
    public OAuth2AccessToken getAccessToken(OAuth2Authentication authentication) {
        ...
        if (accessToken != null
                && !key.equals(customAuthenticationKeyGenerator.extractKey(readAuthentication(accessToken.getValue())))) {
    ...
    public void storeAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication) {
        ...
        jdbcTemplate.update(insertAccessTokenSql, new Object[] { extractTokenKey(token.getValue()),
                new SqlLobValue(serializeAccessToken(token)), customAuthenticationKeyGenerator.extractKey(authentication),
